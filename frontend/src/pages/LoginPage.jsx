...
export default function LoginPage({ onLogin }) {
  ...
  const handleLogin = async (e) => {
    ...
    setError('')
    try {
      const { data } = await api.post('/token/', { username: username.trim(), password })
      ...
      } else setError(t('login_error') || 'فشل تسجيل الدخول. تحقق من البيانات.')
    } finally {
      setSubmitting(false)
    }
  }
  ...
  const Pill = ({ code, onClick }) => (
    <Chip
      label={code}
      onClick={onClick}
      clickable                  // ✅ يجعلها تلتقط النقرات على كامل المساحة
      component="button"         // ✅ عنصر زر حقيقي لتحسين الوصولية
      size="small"
      sx={{
        borderColor: 'divider',
        bgcolor: 'background.paper',
        fontWeight: 600,
        zIndex: 3,               // ✅ فوق أي طبقة مجاورة
      }}
    />
  )
    ...
    >
      ...
      <Box
        sx={{
          ...
          bgcolor: 'background.default',
          zIndex: 2,                 // ✅ يضمن أن العمود فوق أي عناصر مجاورة
        }}
      >
        <Box sx={{ width: '100%', maxWidth: 420 }}>
          ...
          <Stack direction="row" alignItems="center" justifyContent="space-between" sx={{ mb: 3 }}>
            <Stack direction="row" alignItems="center" spacing={1.2}>
              {/* ✅ استبدلنا Avatar النصي بصورة الأيقونة */}
              <Avatar
                variant="rounded"
                src="/assets/icon.svg"
                alt="IBLA Dish Logo"
      ...
      <Box
        sx={{
          ...
          overflow: 'hidden',        // ✅ يمنع أي عناصر مطلقة من الخروج من العمود
        }}
      >
        {/* إزالة الضوء البرتقالي */}
        <Box
          sx={{
            pointerEvents: 'none',
            position: 'absolute',
        ...
        <Typography
          ...
          sx={{
            ...
            textAlign: 'center',
            px: 2,
            pointerEvents: 'none',     // ✅ لا يأخذ نقرات
            zIndex: 0,                 // ✅ خلف كل شيء في العمود
          }}
        >
          IBLA&nbsp;DISH
        </Typography>
