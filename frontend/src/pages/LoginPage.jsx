...
// -----------------------------------------------------------------------------
// Arcana-style Login (UI) + Production logic
// - يستدعي /api/token/ ثم يخزّن access/refresh في localStorage (متوافق مع axios.js)
// - يستدعي /api/auth/whoami لتحديد الدور بدقة (admin / owner)
// - يحفظ role في sessionStorage + localStorage ليستفيد منها AdminRoute/PrivateRoute
// - يعرض رسالة خطأ ودون أي Redirect عند فشل الدخول
// -----------------------------------------------------------------------------
...
export default function LoginPage({ onLogin }) {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      // 2) خزّن التوكنات بما يتوافق مع axios.js (الذي يقرأ من localStorage)
      localStorage.setItem('access_token', accessToken);
      if (refreshToken) localStorage.setItem('refresh_token', refreshToken);

      // خزن أيضًا للراوتر والحمايات
      sessionStorage.setItem('token', accessToken);

      // حدّث الهيدر الافتراضي
      api.defaults.headers.common.Authorization = `Bearer ${accessToken}`;
      setToken(accessToken);
      onLogin?.(accessToken);

      // 3) حدد الدور عبر whoami (أدق)، وإن فشل استخدم الـ JWT claims
      let role = 'owner';
      try {
        const who = await api.get('/auth/whoami');
        const r =
          String(who?.data?.role || '').toLowerCase() ||
          (who?.data?.is_staff ? 'admin' : 'owner');
      ...
      // 4) خزّن الدور ليستفيد منه AdminRoute/PrivateRoute
      ...
      // لا Redirect هنا — نظهر رسالة الخطأ فقط
      if (axios.isAxiosError && axios.isAxiosError(err)) {
        const status = err.response?.status;
        if (status === 400 || status === 401) {
          setError(t('bad_credentials') || 'بيانات الدخول غير صحيحة.');
        } else if (status >= 500) {
          setError(t('server_down') || 'خطأ من الخادم. حاول لاحقًا.');
        } else {
          setError(t('network_error') || 'خطأ شبكة. تحقق من الاتصال.');
        }
      } else {
