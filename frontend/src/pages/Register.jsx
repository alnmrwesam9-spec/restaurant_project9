...
// -----------------------------------------------------------------------------
// Register page — باستخدام أيقونة/لوغو حقيقيين بدلاً من البادجات
// -----------------------------------------------------------------------------
...
import {
  ...
  Slide,
  Chip,
  Avatar, // ⬅️ نستخدمه لعرض الأيقونة
} from "@mui/material";
...
export default function Register({ onLogin }) {
  ...
  useEffect(() => {
    ...
    if (!u) { setUserAvailable(null); setCheckingUser(false); return; }
    const id = setTimeout(async () => {
      setCheckingUser(true);
      try {
        const { data } = await api.get(`/auth/username-available/?username=${encodeURIComponent(u)}`);
  ...
  useEffect(() => {
    ...
    if (!e) { setEmailAvailable(null); setCheckingEmail(false); return; }
    const id = setTimeout(async () => {
      setCheckingEmail(true);
      try {
        const { data } = await api.get(`/auth/email-available/?email=${encodeURIComponent(e)}`);
  ...
  useEffect(() => {
    ...
    if (!pwd) { setPwdScore(0); setPwdMsg(""); return; }
    if (zx) {
      const res = zx(pwd);
    ...
    try {
      ...
      if (data && typeof data === "object") {
        for (const k of [
          ...
          "non_field_errors", "detail", "role",
        ]) {
          if (data[k]) fe[k] = Array.isArray(data[k]) ? data[k].join(" ") : String(data[k]);
        }
        msg = fe.detail || fe.non_field_errors || fe.password || fe.email || fe.username || data.error || msg;
      }
      setFieldErrors(fe);
      setError(msg);
    } finally {
  ...
  // === عناصر صغيرة لمطابقة ستايل صفحة الدخول ===
        ...
        <Box sx={{ width: "100%", maxWidth: 420 }}>
          ...
          <Stack direction="row" alignItems="center" justifyContent="space-between" sx={{ mb: 3 }}>
            ...
            <Stack direction="row" alignItems="center" spacing={1.2}>
              ...
              />
              <Box>
                <Box component="img" src="/assets/logo.svg" alt="IBLA Dish" sx={{ height: 80 }} />
                <Typography variant="caption" color="text.secondary">
                  {t('tagline') || 'Restaurant Management Made Simple'}
                </Typography>
              </Box>
            </Stack>
            ...
            <Slide in direction={isRTL ? 'left' : 'right'} timeout={900}>
              <TextField
                ...
                InputProps={{ endAdornment: (
                  checkingUser ? (
                    <InputAdornment position="end"><HourglassEmptyIcon fontSize="small" /></InputAdornment>
                  ) : userAvailable === true ? (
                    <InputAdornment position="end"><CheckCircleOutlineIcon color="success" fontSize="small" /></InputAdornment>
                  ) : userAvailable === false ? (
                    <InputAdornment position="end"><CancelOutlinedIcon color="error" fontSize="small" /></InputAdornment>
                  ) : null
                ) }}
                helperText={
                  fieldErrors.username ||
                  (userAvailable === false ? t("username_taken") || "اسم المستخدم مستخدم بالفعل." : " ")
                }
            ...
            <Stack spacing={1.2}>
              <Slide in direction={isRTL ? 'left' : 'right'} timeout={900}>
                <Typography variant="body2">
                  • {t('feature_menu') || 'Menu management system'}
                </Typography>
              </Slide>
              <Slide in direction={isRTL ? 'left' : 'right'} timeout={1000}>
                <Typography variant="body2">
