...
export default function MenusPageView() {
  ...
  // ---------- صورة آمنة بفولباك ثابت ومنع حلقة onError ----------
  const SafeImg = React.forwardRef(function SafeImg(
    { src, alt, fallback, onError, ...props },
    ref
  ) {
    const handleErr = (e) => {
      // أوقف الحلقة نهائيًا قبل التحويل إلى البليس هولدر
        ...
        >
          ...
          >
            ...
            <Autocomplete
              ...
              sx={{ flex: 1, bgcolor: 'white', borderRadius: 1, minWidth: 0 }}
              renderInput={(params) => (
            ...
            <Stack direction={{ xs: 'column', md: 'row' }} spacing={1.25} alignItems={{ md: 'center' }} justifyContent="space-between">
              ...
              <Stack direction="row" spacing={1} sx={{ width: '100%', maxWidth: 520 }}>
                ...
                <Button
                  ...
                  startIcon={<AddIcon />}
                  variant="contained"
                  sx={{ borderRadius: 999, px: 2, bgcolor: '#111', boxShadow: 'none', '&:hover': { bgcolor: '#000' } }}
                >
                  {t('actions.add')}
                </Button>
              </Stack>
          ...
          {/* حالات Excel/Errors/Loading */}
          {excelBusy && !excelDialog && (
            <Grid container sx={{ my: 2, overflowX: 'clip' }}>
              <Grid item xs={12}>
          ...
          {excelError && (
            <Grid container sx={{ overflowX: 'clip' }}>
              <Grid item xs={12}>
          ...
          {loading ? (
            ...
            <Grid container spacing={2} alignItems="stretch" justifyContent="center" sx={{ overflowX: 'clip' }}>
              <Grid item xs={12}>
                <Box sx={{ maxWidth: CONTENT_MAX, mx: 'auto', minWidth: 0 }}>
                  {/* بطاقة القائمة المختارة */}
                  <Card
                    variant="outlined"
                    sx={{
                      borderRadius: 3,
                  ...
                  >
                    <Grid container sx={{ overflowX: 'clip' }}>
                      <Grid item xs={12} md={7}>
                        <CardContent sx={{ pb: 2 }}>
                          ...
                          {/* صف الأزرار الأول — تمرير أفقي على xs */}
                          <Box sx={{ display: { xs: 'block', sm: 'none' }, mb: 1 }}>
                            <Box sx={scrollRowSx}>
                              <Button
                                ...
                                variant="contained"
                                size="small"
                                sx={{ ...pillSx, bgcolor: '#111', boxShadow: 'none', '&:hover': { bgcolor: '#000' } }}
                              >
                          ...
                          <Stack direction="row" spacing={1} flexWrap="wrap" alignItems="center" sx={{ mb: 1, display: { xs: 'none', sm: 'flex' } }}>
                            <Button
                              ...
                              variant="contained"
                              size="small"
                              sx={{ borderRadius: 999, bgcolor: '#111', boxShadow: 'none', '&:hover': { bgcolor: '#000' } }}
                            >
                              {t('actions.go_to_menu')}
                            </Button>
                            <Button
                          ...
                          {/* صف الأزرار الثاني — تمرير أفقي على xs فقط */}
                      ...
                      {/* صورة الـHero الجانبية — مُخفاة على الموبايل */}
                      {displayLoading && (
                        <Grid
                          item
                          xs={12}
                    ...
                    {/* الصورة الكبيرة أسفل الأزرار (تبقى على كل المقاسات) */}
                  ...
                  <Box mt={4}>
                    {sections.map((section) => (
                      <Box key={section.id} mb={4}>
                        ...
                        {/* موبايل: تمرير أفقي — شاشات أكبر: Grid */}
                        <Box sx={{ display: { xs: 'block', sm: 'none' } }}>
                          <Box sx={cardsHScrollSx}>
                        ...
                        <Grid container spacing={2} alignItems="stretch" sx={{ display: { xs: 'none', sm: 'flex' }, overflowX: 'clip' }}>
                          {(dishesBySection[section.id] || []).map((dish, idx) => {
                            const { primary } = pickPrimarySecondary(dish);
                            const code = dish?.display_codes || dish?.allergy_info || '';
                            const explain = (dish?.allergen_explanation_de || '').trim();
          ...
          <Dialog open={dishOpen} onClose={() => setDishOpen(false)} maxWidth="sm" fullWidth
            ...
            <DialogActions>
              <Button onClick={() => setDishOpen(false)} variant="contained" sx={{ borderRadius: 999, bgcolor: '#111', boxShadow: 'none', '&:hover': { bgcolor: '#000' } }}>
                {t('actions.close')}
              </Button>
            </DialogActions>
          </Dialog>

          {/* Dialog توليد الأكواد */}
              ...
              <Stack spacing={2}>
                ...
                <Stack direction={{ xs: 'column', sm: 'row' }} spacing={2}>
                  ...
                  <Tooltip title={t('tooltips.use_llm')}>
                    <FormControlLabel control={<Switch checked={genUseLLM} onChange={(e) => setGenUseLLM(e.target.checked)} />} label="use_llm" />
                  </Tooltip>
                ...
                <Grid container spacing={2}>
                  ...
                  <Grid item xs={12} sm={6}>
                    <Tooltip title={t('tooltips.llm_model')}>
                      <TextField fullWidth label="llm_model" value={genModel} onChange={(e) => setGenModel(e.target.value)} />
                    </Tooltip>
                  </Grid>
                  <Grid item xs={6} sm={3}>
                    <Tooltip title={t('tooltips.llm_max_terms')}>
                      <TextField fullWidth type="number" label="llm_max_terms" value={genMaxTerms} onChange={(e) => setGenMaxTerms(e.target.value)} inputProps={{ min: 1, max: 20 }} />
                    </Tooltip>
                  </Grid>
                  <Grid item xs={6} sm={3}>
                    <Tooltip title={t('tooltips.llm_temperature')}>
                      <TextField fullWidth type="number" label="llm_temperature" value={genTemperature} onChange={(e) => setGenTemperature(e.target.value)} inputProps={{ step: 0.1, min: 0, max: 1 }} />
                    </Tooltip>
                  </Grid>
                </Grid>
                ...
                <Stack direction="row" spacing={1}>
                  <Tooltip title={t('tooltips.preview')}>
                    <span>
                      <Button variant="outlined" startIcon={<RefreshIcon />} onClick={previewGenerate} disabled={genBusy} sx={{ borderRadius: 999 }}>
                  ...
                  <Tooltip title={t('tooltips.run')}>
                    <span>
                      <Button variant="contained" startIcon={<ScienceIcon />} onClick={runGenerate} disabled={genBusy}
                              sx={{ borderRadius: 999, bgcolor: '#111', boxShadow: 'none', '&:hover': { bgcolor: '#000' } }}>
                    ...
                    {Array.isArray(filteredLLMItems) && filteredLLMItems.length > 0 ? (
                      <Box sx={{ border: '1px dashed', borderColor: 'divider', borderRadius: 1, p: 1 }}>
                        ...
                        <Stack direction={{ xs: 'column', sm: 'row' }} spacing={1} alignItems={{ sm: 'center' }}>
                          ...
                          <Button variant="contained" disabled={upsertBusy || selectedCount === 0} onClick={handleUpsertSelected}
                                  sx={{ borderRadius: 999, bgcolor: '#111', boxShadow: 'none', '&:hover': { bgcolor: '#000' } }}>
