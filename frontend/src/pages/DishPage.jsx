    ...
    >
      ...
      <Grid container spacing={2} alignItems="flex-start">
        ...
        <Grid item xs={12} md={4} sx={{ minWidth: 0 }}>
          <Card variant="outlined" sx={{ borderRadius: 3, p: { xs: 2, md: 2 }, position: { md: 'sticky' }, top: { xs: 8, sm: 72 } }}>
            ...
            <Stack spacing={1.25}>
              ...
              <TextField size="small" label={t('dish_name')} name="name" value={newDish.name} onChange={handleInputChange} fullWidth />
              ...
              {/* رفع صورة */}
              <Box sx={{ border: '1px dashed', borderColor: 'divider', borderRadius: 2, p: 2, textAlign: 'center', bgcolor: 'background.default' }}>
              ...
              <Button fullWidth variant="contained" color="primary" startIcon={<SaveIcon />} onClick={handleAddOrEditDish} sx={{ borderRadius: 2 }}>
        ...
        <Grid item xs={12} md={8} sx={{ minWidth: 0 }}>
          {/* شريط البحث — بدون تلوين مخصص لتجنب مشاكل stylis */}
          <Box sx={{ mb: 2 }}>
            <TextField
              fullWidth
              size="small"
          ...
          {/* بانر — خلفية ثابتة بدل gradient */}
          <Card sx={{ borderRadius: 3, mb: 2, overflow: 'hidden', bgcolor: 'grey.900', color: 'common.white' }}>
            <CardContent sx={{ py: { xs: 3, md: 4 } }}>
              <Typography variant="h6" sx={{ fontWeight: 800, mb: 1 }}>
                {t('Welcome to the dish editing page') || 'Welcome to IBLA, Admin!'}
              </Typography>
              <Typography variant="body2" sx={{ opacity: 0.85 }}>
                {t('From here you can edit all the dishes at any time.') || 'Manage your restaurant’s dishes with ease. Add new items, update prices, and keep your menu fresh.'}
              </Typography>
              <Box sx={{ mt: 2, display: 'inline-flex', alignItems: 'center', gap: 1, bgcolor: 'rgba(255,255,255,.08)', borderRadius: 999, px: 1.5, py: 0.75 }}>
                ...
                {filteredDishes.map((dish) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      >
                        {dish.image ? (
                          <CardMedia component="img" image={dish.image} alt={dish.name}
          ...
          {previewDish && (
            <Stack spacing={2}>
              <Box sx={{ width: '100%', height: 260, borderRadius: 2, overflow: 'hidden', bgcolor: 'grey.100' }}>
                {previewDish.image ? (
                  <Box component="img" src={previewDish.image} alt={previewDish.name} sx={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                ) : (
                  <Box sx={{ width: '100%', height: '100%', display: 'grid', placeItems: 'center', color: 'text.disabled' }}>
