...
// - كل الألوان من الثيم (IBLA DISH): primary + palette/alpha
// - إبراز اللون الأساسي بوضوح في الرسوم والـ CTA
// - لا تغيير على أي API
...
const StatCard = ({ title, value, delta, delay = 0 }) => (
  <motion.div variants={fadeUp} transition={{ duration: 0.45, ease: 'easeOut', delay }}>
    <Card variant="outlined" sx={{ borderRadius: 3, bgcolor: 'background.paper', borderColor: 'divider' }}>
      <CardContent>
...
const LineChartLite = () => {
  ...
  // إبراز البرتقالي الأساسي + لون ثانوي من الثيم
  const series = useMemo(() => ([
    { d: 'M0,160 C100,80 180,120 260,70 C340,30 420,110 510,70 C560,55 600,90 600,90', color: theme.palette.primary.main },
    { d: 'M0,180 C100,120 180,160 260,110 C340,80 420,160 510,120 C560,110 600,150 600,150', color: theme.palette.success.main },
  ]), [theme.palette.primary.main, theme.palette.success.main]);

  const grid = alpha(theme.palette.divider, 0.7);
...
const DonutTop3 = () => {
  ...
  const holeBg = (theme.vars?.palette?.background?.paper) || theme.palette.background.paper;
  ...
  return (
    ...
    >
      <svg viewBox="0 0 220 220" width="100%" height="100%">
        <circle cx={cx} cy={cy} r={r} fill="none" stroke={alpha(theme.palette.divider, 0.5)} strokeWidth="24" />
        {segments.map((s, i) => (
          <motion.circle
            key={i}
            cx={cx} cy={cy} r={r}
    ...
    <Container maxWidth="lg" sx={{ py: 3 }}>
      ...
      <Stack component={motion.div} variants={fade} initial="hidden" animate="visible"
        ...
        <Stack direction="row" spacing={1} alignItems="center">
          ...
          <Button variant="contained" color="primary" startIcon={<FileDownloadIcon />} sx={{ borderRadius: 2, boxShadow: 'none' }}>
            {t('export') || t('export_excel') || 'Export'}
          </Button>
          <Avatar sx={{ width: 32, height: 32, bgcolor: 'primary.main', color: 'primary.contrastText' }}>ID</Avatar>
        </Stack>
      </Stack>
      ...
      <Grid container spacing={2} mb={2}>
        <Grid item xs={12} md={8}>
          <motion.div variants={fadeUp} initial="hidden" whileInView="visible" viewport={{ once: true, amount: 0.2 }}
            ...
            <Card variant="outlined" sx={{ borderRadius: 3, height: '100%', bgcolor: 'background.paper', borderColor: 'divider' }}>
              <CardContent>
            ...
            <Card variant="outlined" sx={{ borderRadius: 3, height: '100%', bgcolor: 'background.paper', borderColor: 'divider' }}>
              <CardContent>
                ...
                <Stack direction="row" spacing={1} justifyContent="center" mt={2}>
                  <Chip size="small" label="Sushi" sx={{ bgcolor: theme.palette.primary.main, color: theme.palette.primary.contrastText }} />
                  <Chip size="small" label="Burger" sx={{ bgcolor: theme.palette.success.main, color: theme.palette.getContrastText(theme.palette.success.main) }} />
                  <Chip size="small" label="Pasta" sx={{ bgcolor: alpha(theme.palette.text.primary, 0.7), color: theme.palette.background.paper }} />
                </Stack>
              </CardContent>
            </Card>
          </motion.div>
      ...
      <motion.div variants={fadeUp} initial="hidden" whileInView="visible" viewport={{ once: true, amount: 0.2 }}
                  transition={{ duration: 0.45, ease: 'easeOut' }}>
        <Card variant="outlined" sx={{ borderRadius: 3, mb: 2, bgcolor: 'background.paper', borderColor: 'divider' }}>
          <CardContent>
            <Typography variant="subtitle1" fontWeight={800} mb={1}>{t('recent_orders')}</Typography>
            <Paper variant="outlined" sx={{ borderRadius: 2, overflow: 'hidden', borderColor: 'divider', bgcolor: 'background.paper' }}>
              <Table size="small">
                <TableHead>
                  <TableRow>
      ...
      <Grid container spacing={2}>
        <Grid item xs={12} md={8}>
          <motion.div variants={fadeUp} initial="hidden" whileInView="visible" viewport={{ once: true, amount: 0.2 }}
                      transition={{ duration: 0.45, ease: 'easeOut' }}>
            <Card variant="outlined" sx={{ borderRadius: 3, overflow: 'hidden', borderColor: 'divider', bgcolor: 'background.paper' }}>
              <motion.div initial={{ scale: 1.02 }} animate={{ scale: 1 }} transition={{ duration: 1.2, ease: 'easeOut' }}>
                <Box sx={{
                  height: 210,
                  backgroundImage: 'url(https://images.unsplash.com/photo-1550317138-10000687a72b?q=80&w=1400&auto=format&fit=crop)',
          ...
          <motion.div variants={fadeUp} initial="hidden" whileInView="visible" viewport={{ once: true, amount: 0.2 }}
            ...
            <Card variant="outlined" sx={{ borderRadius: 3, overflow: 'hidden', borderColor: 'divider', bgcolor: 'background.paper' }}>
              <motion.div initial={{ scale: 1.02 }} animate={{ scale: 1 }} transition={{ duration: 1.2, ease: 'easeOut' }}>
                <Box sx={{
                  height: 210,
                  backgroundImage: 'url(https://images.unsplash.com/photo-1505685296765-3a2736de412f?q=80&w=1200&auto=format&fit=crop)',
                  backgroundSize: 'cover', backgroundPosition: 'center'
                }} />
              </motion.div>
              <CardContent>
