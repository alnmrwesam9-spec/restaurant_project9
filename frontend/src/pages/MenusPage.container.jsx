...
function MenusPage({ token }) {
  ...
  const parseWorkbookToPreview = (wb) => {
    const previews = [];
  ...
  const handleExcelChosenKeepWB = async (ev) => {
    ...
    try {
      ...
      setSheetsPreview(parseWorkbookToPreview(wb));
      setExcelDialog(true);
      setWbBinary(data);
    } catch {
  ...
  const doImportNow = async () => {
    ...
    try {
      ...
      const menuRes = await api.post('/menus/', { name: importMenuName || t('import.default_menu_name') });
      ...
      for (const s of enabledSheets) {
        const secRes = await api.post('/sections/', { name: s.name, menu: menuId });
      ...
      for (const s of enabledSheets) {
        ...
        for (const row of rows) {
          ...
          try {
            const dishRes = await api.post('/dishes/', { name, description, allergy_info, section: sectionIdByName[s.name] });
            const dishId = dishRes?.data?.id;
            if (dishId && price != null) {
              await api.post(`/v2/dishes/${dishId}/prices/`, { label: '', price, is_default: true, sort_order: 0 });
            }
          } catch {}
        }
      }
